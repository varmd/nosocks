...
#define MAXAPPS         25            /* Max length of conf line  */

//Max 100 apps allowed
char apps_allowed_names[MAXAPPS][100+1];
...
static int handle_apps(struct parsedfile *, int, char *);
...
int read_config (char *filename, struct parsedfile *config) {
    ...
    */
	...
	if ((conf = fopen(filename, "r")) == NULL) {
      ...
      handle_local(config, 0, "0.0.0.0/255.0.0.0");
...
static int handle_line(struct parsedfile *config, char *line, int lineno) {
	...
	if (nowords > 0) {
		...
		if (!strcmp(words[0], "path")) {
			...
			} else if (!strcmp(words[0], "app")) {
				handle_apps(config, lineno, words[2]);
			} else {
...
static int handle_apps(struct parsedfile *config, int lineno, char *value) {
	

  static int counter = 0;
  
  if(value) {
    strcpy(apps_allowed_names[counter], value);
  }
